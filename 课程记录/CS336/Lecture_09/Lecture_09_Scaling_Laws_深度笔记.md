...
**X 轴：** Parameters (non-embedding)，从 $`10^5`$ 到 $`10^9`$<br>
...
**X 轴：** Training Data Set Size, Number of Chars（字符数，对数刻度，$`2^{19}`$ 到 $`2^{27}`$）<br>
...
**X 轴：** Parameters (non-embedding)，从 $`10^3`$ 到 $`10^9`$<br>
**Y 轴：** Test Loss（2 到 7）
...
**X 轴：** WebText2 Train Loss（从 $`3 \times 10^0`$ 到 $`10^1`$，对数刻度）<br>
